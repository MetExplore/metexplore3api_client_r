#' Create a new GraphsResponse
#'
#' @description
#' GraphsResponse Class
#'
#' @docType class
#' @title GraphsResponse
#' @description GraphsResponse Class
...
#' @field results Array of Grahs list(\link{Graph}) [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
GraphsResponse <- R6::R6Class(
  "GraphsResponse",
  public = list(
    `message` = NULL,
    `success` = NULL,
    ...
    #' @description
    #' Initialize a new GraphsResponse class.
    #'
    ...
    #' To JSON String
    #'
    #' @return GraphsResponse in JSON format
    toJSON = function() {
      GraphsResponseObject <- list()
      if (!is.null(self$`message`)) {
        GraphsResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        GraphsResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        GraphsResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`results`)) {
        GraphsResponseObject[["results"]] <-
          lapply(self$`results`, function(x) x$toJSON())
      }
      GraphsResponseObject
    },

    #' @description
    #' Deserialize JSON string into an instance of GraphsResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of GraphsResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Graph]", loadNamespace("metexplore3api?"))
      }
      self
    },
...
',
    ...
    #' @description
    #' Validate JSON input with respect to GraphsResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GraphsResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GraphsResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GraphsResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of GraphsResponse
    toString = function() {
      self$toJSONString()
    },
...
# GraphsResponse$unlock()
#
## Below is an example to define the print function
# GraphsResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
