#' Create a new VizArrayResponse
#'
#' @description
#' VizArrayResponse Class
#'
#' @docType class
#' @title VizArrayResponse
#' @description VizArrayResponse Class
...
#' @field results Array of Viz list(\link{Viz}) [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
VizArrayResponse <- R6::R6Class(
  "VizArrayResponse",
  public = list(
    `message` = NULL,
    `success` = NULL,
    ...
    #' @description
    #' Initialize a new VizArrayResponse class.
    #'
    ...
    #' To JSON String
    #'
    #' @return VizArrayResponse in JSON format
    toJSON = function() {
      VizArrayResponseObject <- list()
      if (!is.null(self$`message`)) {
        VizArrayResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        VizArrayResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        VizArrayResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`results`)) {
        VizArrayResponseObject[["results"]] <-
          lapply(self$`results`, function(x) x$toJSON())
      }
      VizArrayResponseObject
    },

    #' @description
    #' Deserialize JSON string into an instance of VizArrayResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of VizArrayResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Viz]", loadNamespace("metexplore3api"))
      }
      self
    },
...
',
    ...
    #' @description
    #' Validate JSON input with respect to VizArrayResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for VizArrayResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for VizArrayResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for VizArrayResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of VizArrayResponse
    toString = function() {
      self$toJSONString()
    },
...
# VizArrayResponse$unlock()
#
## Below is an example to define the print function
# VizArrayResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# VizArrayResponse$lock()
