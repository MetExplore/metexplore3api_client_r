...
JobResponse <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`message`, `success`, `status`, `job` = NULL, ...) {
      if (!missing(`message`)) {
    ...
    toJSON = function() {
      ...
      if (!is.null(self$`message`)) {
        JobResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        JobResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        JobResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`job`)) {
        JobResponseObject[["job"]] <-
          self$`job`$toJSON()
      }
      JobResponseObject
    },
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`job`)) {
        `job_object` <- Job$new()
        `job_object`$fromJSON(jsonlite::toJSON(this_object$`job`, auto_unbox = TRUE, digits = NA))
        self$`job` <- `job_object`
      }
      self
    },
    ...
    fromJSONString = function(input_json) {
      ...
      self$`success` <- this_object$`success`
      self$`status` <- this_object$`status`
      self$`job` <- Job$new()$fromJSON(jsonlite::toJSON(this_object$`job`, auto_unbox = TRUE, digits = NA))
      self
    },
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for JobResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for JobResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for JobResponse: the required field `status` is missing."))
      }
    },
