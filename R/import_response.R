...
#' @description ImportResponse Class
...
ImportResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' To JSON String
    #'
    #' @return ImportResponse in JSON format
    toJSON = function() {
      ImportResponseObject <- list()
      if (!is.null(self$`message`)) {
        ImportResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        ImportResponseObject[["success"]] <-
          self$`success`
      }
    ...
    #' @description
    #' Deserialize JSON string into an instance of ImportResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of ImportResponse
    ...
    #' @description
    #' Validate JSON input with respect to ImportResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ImportResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ImportResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ImportResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of ImportResponse
    toString = function() {
      self$toJSONString()
    },
...
# ImportResponse$unlock()
#
## Below is an example to define the print function
# ImportResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
