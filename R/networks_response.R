...
#' @description NetworksResponse Class
...
#' @field networks Array of Networks list(\link{Network}) [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
NetworksResponse <- R6::R6Class(
  "NetworksResponse",
  public = list(
    `message` = NULL,
    `success` = NULL,
    ...
    #' @description
    #' Initialize a new NetworksResponse class.
    #'
    ...
    initialize = function(`message`, `success`, `status`, `networks` = NULL, ...) {
      if (!missing(`message`)) {
      ...
      if (!is.null(`networks`)) {
        stopifnot(is.vector(`networks`), length(`networks`) != 0)
        sapply(`networks`, function(x) stopifnot(R6::is.R6(x)))
    ...
    #' To JSON String
    #'
    #' @return NetworksResponse in JSON format
    toJSON = function() {
      NetworksResponseObject <- list()
      if (!is.null(self$`message`)) {
        NetworksResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        NetworksResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        NetworksResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`networks`)) {
        NetworksResponseObject[["networks"]] <-
          lapply(self$`networks`, function(x) x$toJSON())
      }
      NetworksResponseObject
    },

    #' @description
    #' Deserialize JSON string into an instance of NetworksResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of NetworksResponse
    ...
    toJSONString = function() {
      jsoncontent <- c(
        ...
        if (!is.null(self$`networks`)) {
          ...
          [%s]
',
          paste(sapply(self$`networks`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox = TRUE, digits = NA)), collapse = ",")
          )
        }
      )
    ...
    #' @description
    #' Validate JSON input with respect to NetworksResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for NetworksResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for NetworksResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for NetworksResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of NetworksResponse
    toString = function() {
      self$toJSONString()
    },
...
# NetworksResponse$unlock()
#
## Below is an example to define the print function
# NetworksResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# NetworksResponse$lock()
