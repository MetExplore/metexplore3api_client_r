...
#' @docType class
#' @title JobsResponse
#' @description JobsResponse Class
...
#' @field results Array of jobs list(\link{Job}) [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
JobsResponse <- R6::R6Class(
  "JobsResponse",
  public = list(
    `message` = NULL,
    `success` = NULL,
    ...
    #' @description
    #' Initialize a new JobsResponse class.
    #'
    ...
    #' To JSON String
    #'
    #' @return JobsResponse in JSON format
    toJSON = function() {
      JobsResponseObject <- list()
      if (!is.null(self$`message`)) {
        JobsResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        JobsResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        JobsResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`results`)) {
        JobsResponseObject[["results"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of JobsResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of JobsResponse
...
',
    ...
    #' @description
    #' Validate JSON input with respect to JobsResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for JobsResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for JobsResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for JobsResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of JobsResponse
    toString = function() {
      self$toJSONString()
    },
...
# JobsResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
