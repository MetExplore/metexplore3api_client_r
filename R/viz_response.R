...
#' @field result  \link{Viz} [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
VizResponse <- R6::R6Class(
  "VizResponse",
  public = list(
    `message` = NULL,
    `success` = NULL,
    ...
    #' @description
    #' Initialize a new VizResponse class.
    #'
    ...
    #' To JSON String
    #'
    #' @return VizResponse in JSON format
    toJSON = function() {
      VizResponseObject <- list()
      if (!is.null(self$`message`)) {
        VizResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        VizResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        VizResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`result`)) {
        VizResponseObject[["result"]] <-
          self$`result`$toJSON()
      }
      VizResponseObject
    },
    ...
    #' @description
    #' Deserialize JSON string into an instance of VizResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of VizResponse
    ...
    fromJSONString = function(input_json) {
      ...
      self$`success` <- this_object$`success`
      self$`status` <- this_object$`status`
      self$`result` <- Viz$new()$fromJSON(jsonlite::toJSON(this_object$`result`, auto_unbox = TRUE, digits = NA))
      self
    },

    #' @description
    #' Validate JSON input with respect to VizResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for VizResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for VizResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for VizResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of VizResponse
    toString = function() {
      self$toJSONString()
    },
...
# VizResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
