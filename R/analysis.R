...
Analysis <- R6::R6Class(
  ...
  public = list(
    ...
    isValid = function() {
      ...
      if (!str_detect(self$`date_edition`, "^\\d{4}-\\d{2}-\\d{2}\\s\\d{2}:\\d{2}:\\d{2}$")) {
        return(FALSE)
      }
    ...
    getInvalidFields = function() {
      ...
      if (!str_detect(self$`date_edition`, "^\\d{4}-\\d{2}-\\d{2}\\s\\d{2}:\\d{2}:\\d{2}$")) {
        invalid_fields["date_edition"] <- "Invalid value for `date_edition`, must conform to the pattern ^\\d{4}-\\d{2}-\\d{2}\\s\\d{2}:\\d{2}:\\d{2}$."
      }
