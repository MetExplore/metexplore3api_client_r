#' Create a new EnzymesResponse
#'
#' @description
#' EnzymesResponse Class
#'
#' @docType class
#' @title EnzymesResponse
#' @description EnzymesResponse Class
...
#' @field results Array of Enzyme list(\link{Enzyme}) [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
EnzymesResponse <- R6::R6Class(
  "EnzymesResponse",
  public = list(
    `message` = NULL,
    `success` = NULL,
    ...
    #' @description
    #' Initialize a new EnzymesResponse class.
    #'
    ...
    #' To JSON String
    #'
    #' @return EnzymesResponse in JSON format
    toJSON = function() {
      EnzymesResponseObject <- list()
      if (!is.null(self$`message`)) {
        EnzymesResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        EnzymesResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        EnzymesResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`results`)) {
        EnzymesResponseObject[["results"]] <-
          lapply(self$`results`, function(x) x$toJSON())
      }
      EnzymesResponseObject
    },

    #' @description
    #' Deserialize JSON string into an instance of EnzymesResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of EnzymesResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Enzyme]", loadNamespace("metexplore3api?"))
      }
      self
    },
...
',
    ...
    #' @description
    #' Validate JSON input with respect to EnzymesResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for EnzymesResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for EnzymesResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for EnzymesResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of EnzymesResponse
    toString = function() {
      self$toJSONString()
    },
...
# EnzymesResponse$unlock()
#
## Below is an example to define the print function
# EnzymesResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# EnzymesResponse$lock()
