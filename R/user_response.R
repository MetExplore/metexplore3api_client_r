...
#' @docType class
#' @title UserResponse
#' @description UserResponse Class
...
UserResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new UserResponse class.
    #'
    ...
    initialize = function(`message`, `success`, `status`, `user` = NULL, ...) {
      if (!missing(`message`)) {
    ...
    #' To JSON String
    #'
    #' @return UserResponse in JSON format
    toJSON = function() {
      UserResponseObject <- list()
      if (!is.null(self$`message`)) {
        UserResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        UserResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        UserResponseObject[["status"]] <-
          self$`status`
      }
    ...
    #' @description
    #' Deserialize JSON string into an instance of UserResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of UserResponse
    ...
    #' @description
    #' Validate JSON input with respect to UserResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for UserResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for UserResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for UserResponse: the required field `status` is missing."))
      }
    },
...
# UserResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
