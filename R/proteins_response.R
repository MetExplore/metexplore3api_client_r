#' Create a new ProteinsResponse
#'
#' @description
#' ProteinsResponse Class
#'
#' @docType class
#' @title ProteinsResponse
#' @description ProteinsResponse Class
...
#' @field results Array of Protein list(\link{Protein}) [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
ProteinsResponse <- R6::R6Class(
  "ProteinsResponse",
  public = list(
    `message` = NULL,
    `success` = NULL,
    ...
    #' @description
    #' Initialize a new ProteinsResponse class.
    #'
    ...
    #' @param results Array of Protein
    ...
    #' To JSON String
    #'
    #' @return ProteinsResponse in JSON format
    toJSON = function() {
      ProteinsResponseObject <- list()
      if (!is.null(self$`message`)) {
        ProteinsResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        ProteinsResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        ProteinsResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`results`)) {
        ProteinsResponseObject[["results"]] <-
          lapply(self$`results`, function(x) x$toJSON())
      }
      ProteinsResponseObject
    },

    #' @description
    #' Deserialize JSON string into an instance of ProteinsResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of ProteinsResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Protein]", loadNamespace("metexplore3api"))
      }
      self
    },
...
',
    ...
    #' @description
    #' Validate JSON input with respect to ProteinsResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ProteinsResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ProteinsResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ProteinsResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of ProteinsResponse
    toString = function() {
      self$toJSONString()
    },
...
# ProteinsResponse$unlock()
#
## Below is an example to define the print function
# ProteinsResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ProteinsResponse$lock()
