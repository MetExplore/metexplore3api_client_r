...
#' @description NetworkResponse Class
...
#' @field network  \link{Network} [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
NetworkResponse <- R6::R6Class(
  "NetworkResponse",
  public = list(
    `message` = NULL,
    `success` = NULL,
    ...
    #' @description
    #' Initialize a new NetworkResponse class.
    #'
    ...
    initialize = function(`message`, `success`, `status`, `network` = NULL, ...) {
      if (!missing(`message`)) {
    ...
    #' To JSON String
    #'
    #' @return NetworkResponse in JSON format
    toJSON = function() {
      NetworkResponseObject <- list()
      if (!is.null(self$`message`)) {
        NetworkResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        NetworkResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        NetworkResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`network`)) {
        NetworkResponseObject[["network"]] <-
          self$`network`$toJSON()
      }
    ...
    #' @description
    #' Deserialize JSON string into an instance of NetworkResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of NetworkResponse
    ...
    fromJSONString = function(input_json) {
      ...
      self$`success` <- this_object$`success`
      self$`status` <- this_object$`status`
      self$`network` <- Network$new()$fromJSON(jsonlite::toJSON(this_object$`network`, auto_unbox = TRUE, digits = NA))
      self
    },

    #' @description
    #' Validate JSON input with respect to NetworkResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for NetworkResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for NetworkResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for NetworkResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of NetworkResponse
    toString = function() {
      self$toJSONString()
    },
...
# NetworkResponse$unlock()
#
## Below is an example to define the print function
# NetworkResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# NetworkResponse$lock()
