...
#' @docType class
#' @title CountResponse
#' @description CountResponse Class
...
CountResponse <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new CountResponse class.
    #'
    ...
    initialize = function(`message`, `success`, `status`, `count` = NULL, ...) {
      if (!missing(`message`)) {
    ...
    #' To JSON String
    #'
    #' @return CountResponse in JSON format
    toJSON = function() {
      CountResponseObject <- list()
      if (!is.null(self$`message`)) {
        CountResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        CountResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        CountResponseObject[["status"]] <-
          self$`status`
      }
    ...
    #' @description
    #' Deserialize JSON string into an instance of CountResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of CountResponse
    ...
    #' @description
    #' Validate JSON input with respect to CountResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CountResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CountResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CountResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of CountResponse
    toString = function() {
      self$toJSONString()
    },
...
# CountResponse$unlock()
#
## Below is an example to define the print function
# CountResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
