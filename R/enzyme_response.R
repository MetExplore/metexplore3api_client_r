#' Create a new EnzymeResponse
#'
#' @description
#' EnzymeResponse Class
#'
#' @docType class
#' @title EnzymeResponse
#' @description EnzymeResponse Class
...
#' @field result  \link{Enzyme} [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
EnzymeResponse <- R6::R6Class(
  "EnzymeResponse",
  public = list(
    `message` = NULL,
    `success` = NULL,
    ...
    #' @description
    #' Initialize a new EnzymeResponse class.
    #'
    ...
    #' To JSON String
    #'
    #' @return EnzymeResponse in JSON format
    toJSON = function() {
      EnzymeResponseObject <- list()
      if (!is.null(self$`message`)) {
        EnzymeResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        EnzymeResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        EnzymeResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`result`)) {
        EnzymeResponseObject[["result"]] <-
          self$`result`$toJSON()
      }
      EnzymeResponseObject
    },

    #' @description
    #' Deserialize JSON string into an instance of EnzymeResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of EnzymeResponse
    ...
    fromJSONString = function(input_json) {
      ...
      self$`success` <- this_object$`success`
      self$`status` <- this_object$`status`
      self$`result` <- Enzyme$new()$fromJSON(jsonlite::toJSON(this_object$`result`, auto_unbox = TRUE, digits = NA))
      self
    },

    #' @description
    #' Validate JSON input with respect to EnzymeResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for EnzymeResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for EnzymeResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for EnzymeResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of EnzymeResponse
    toString = function() {
      self$toJSONString()
    },
...
# EnzymeResponse$unlock()
#
## Below is an example to define the print function
# EnzymeResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
