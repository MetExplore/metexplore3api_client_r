...
#' @docType class
#' @title GenesResponse
#' @description GenesResponse Class
...
#' @field results Array of Genes list(\link{Gene}) [optional]
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
GenesResponse <- R6::R6Class(
  "GenesResponse",
  public = list(
    `message` = NULL,
    `success` = NULL,
    ...
    #' @description
    #' Initialize a new GenesResponse class.
    #'
    ...
    #' To JSON String
    #'
    #' @return GenesResponse in JSON format
    toJSON = function() {
      GenesResponseObject <- list()
      if (!is.null(self$`message`)) {
        GenesResponseObject[["message"]] <-
          self$`message`
      }
      if (!is.null(self$`success`)) {
        GenesResponseObject[["success"]] <-
          self$`success`
      }
      if (!is.null(self$`status`)) {
        GenesResponseObject[["status"]] <-
          self$`status`
      }
      if (!is.null(self$`results`)) {
        GenesResponseObject[["results"]] <-
    ...
    #' @description
    #' Deserialize JSON string into an instance of GenesResponse
    #'
    #' @param input_json the JSON input
    #' @return the instance of GenesResponse
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`results`)) {
        self$`results` <- ApiClient$new()$deserializeObj(this_object$`results`, "array[Gene]", loadNamespace("metexplore3api"))
      }
      self
    },
...
',
    ...
    #' @description
    #' Validate JSON input with respect to GenesResponse and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`message`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GenesResponse: the required field `message` is missing."))
      }
      ...
      if (!is.null(input_json$`success`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GenesResponse: the required field `success` is missing."))
      }
      ...
      if (!is.null(input_json$`status`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GenesResponse: the required field `status` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of GenesResponse
    toString = function() {
      self$toJSONString()
    },
...
# GenesResponse$unlock()
#
## Below is an example to define the print function
# GenesResponse$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
